...
- Feather composes the global `global.abort` action from printer activity and page-owned controls. Calibration progress declares itself armed, while G-code leases, `_WAIT_TEMPERATURE`, motion, heater targets, and print state keep ABORT visible on every live page except Home. Touch handling dispatches `M112` before command-depth checks, debounce, or the 80 ms button-feedback callback, so it never waits for the active calibration's G-code mutex.
