...
- Print cancellation uses a separate one-shot `_WAIT_TEMPERATURE.cancel_pending` latch. Feather sets it immediately, even during atomic homing/probing/motion, then queues the ordinary `CANCEL_PRINT`. The current or next managed wait unwinds the enclosing preparation macro; `_STOP` clears the latch after normal cancellation/finish cleanup so a later workflow cannot inherit it. No extra probing or motion cancellation checkpoints are involved, and `M112` remains the immediate emergency path.
