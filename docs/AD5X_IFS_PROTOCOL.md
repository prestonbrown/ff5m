...
- `channel_count` is parsed by `zmod_ifs.py` but **does not appear in this format
  string**. Either it comes from another opcode or it never arrives.
...
32 opcodes, 58 distinct response strings. **ZMOD uses 9.**

| Opcode | Response | Used by ZMOD |
|--------|----------|:---:|
| `F10` | `FFS channel N feeding.` / `FFS channel not exist.` / `FFS not ready.` / `No channel selected.` | yes |
| `F11` | `FFS channel N exiting.` + same error set | yes |
| `F12` | `%d %d %d %d` | no |
| `F13` | the status line above | yes |
| `F14` | `stall: %d %d %d %d` | no |
| `F15` | `F15 ok.` | yes |
| `F18` | `F18 ok` *(no trailing period)* | yes |
| `F19` | `four color. version: 3.0.6` | **no** |
| `F20` | `F20 ok.` | no |
| `F21` | `silk: %d %d %d %d` + `stall: %d %d %d %d` | no |
| `F22` | `ffs_channels_insert: %d` | no |
| `F23` | `chan N.` / `no chan.` | yes |
| `F24` | `chan N.` / `FFS channel not exist.` / `No channel selected.` | yes |
| `F30` | `F30 ok.` | no |
| `F39` | `FFS channel N release.` + error set | yes |
| `F40` | `stall count: C1: %d C2: %d C3: %d C4: %d` | no |
| `F41` | `GCONF: %02x%02x%02x%02x` | no |
| `F42` | `stepper_motor: %d stepper_motor_irun: %d` | no |
| `F43` | `F43 ok.` | no |
| `F44` | `DRV_STATUS: %02x%02x%02x%02x` | no |
| `F45` | `GSTAT: %02x%02x%02x%02x` | no |
...
- **`F19` is a capability probe.** `four color. version: 3.0.6` gives channel
  count *and* firmware version. Every driver today assumes four channels; this
  asks.
...
revisions. Probe `F19` and branch on the version rather than assuming this table
...
- Where `channel_count` comes from is unknown.
